...
pub fn typecheck_pgm(ctx: &mut Ctx, expr: &mut Expr) -> Result<(), TypeErr> {
    ...
    let ty = typecheck(ctx, &mut ty_env, &mut subst_env, &mut scope, expr)?;
...
fn typecheck(
    ...
    match expr {
        ...
        Expr::Not(e) => {
            let e_ty = typecheck(ctx, ty_env, subst_env, scope, e)?;
        ...
        Expr::IntBinOp(e1, _, e2) => {
            let e1_ty = typecheck(ctx, ty_env, subst_env, scope, e1)?;
            let e2_ty = typecheck(ctx, ty_env, subst_env, scope, e2)?;
        ...
        Expr::If(e1, e2, e3) => {
            let e1_ty = typecheck(ctx, ty_env, subst_env, scope, e1)?;
            let e2_ty = typecheck(ctx, ty_env, subst_env, scope, e2)?;
            let e3_ty = typecheck(ctx, ty_env, subst_env, scope, e3)?;
        ...
        Expr::Let {
            ...
            let rhs_ty = typecheck(ctx, ty_env, subst_env, scope, rhs)?;
            ...
            let ret = typecheck(ctx, ty_env, subst_env, scope, body);
            scope.pop_scope();
            ret
        }
        ...
        Expr::LetRec {
            ...
            let rhs_ty_ = typecheck(ctx, ty_env, subst_env, scope, rhs)?;
        ...
        Expr::App { fun, args } => {
            ...
            let mut arg_tys: Vec<Type> = Vec::with_capacity(args.len());
            for arg in args {
                arg_tys.push(typecheck(ctx, ty_env, subst_env, scope, arg)?);
            }
            let fun_ty = Type::Fun {
                args: arg_tys,
                ret: Box::new(ret_ty.clone()),
            };
            let fun_ty_ = typecheck(ctx, ty_env, subst_env, scope, fun)?;
        ...
        Expr::Array { len, elem } => {
            let len_ty = typecheck(ctx, ty_env, subst_env, scope, len)?;
            unify(subst_env, &len_ty, &Type::Int)?;
            let elem_ty = typecheck(ctx, ty_env, subst_env, scope, elem)?;
