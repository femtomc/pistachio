...
double mc_float_of_int_f(FunctionClosure *self, int64_t i) { return (double)i; }
...
double mc_abs_float_f(FunctionClosure *self, double d) { return fabs(d); }

FunctionClosure mc_abs_float = {.function = &mc_abs_float_f};

double mc_sqrt_f(FunctionClosure *self, double d) { return sqrt(d); }

FunctionClosure mc_sqrt = {.function = &mc_sqrt_f};

double mc_sin_f(FunctionClosure *self, double d) { return sin(d); }

FunctionClosure mc_sin = {.function = &mc_sin_f};

double mc_cos_f(FunctionClosure *self, double d) { return cos(d); }
